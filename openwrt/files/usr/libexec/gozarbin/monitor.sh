...
watchdog() {
	local port
	nft list chain inet gozarbin_proxy prerouting >/dev/null 2>&1 || { missing=0; return 0; }
	port=$(uci -q get gozarbin.main.tproxy_port)
	[ -n "$port" ] || port=1821
	...
	missing=0
	logger -t gozarbin "nothing is listening on TProxy port $port; removing the redirect so the LAN keeps working"
