...
#
# Aether's own sing-box core, and the daily look for a newer one.
#
# The core is kept at $own, installed from SagerNet's own releases, and it is
# what Aether runs. A sing-box that came from a Passwall feed is never used and
...
# Aether has no core of its own yet.
#
...
own=/usr/libexec/aether/sing-box
state_dir=/etc/aether/state
...
# The binary Aether runs, or nothing, which callers must treat as "no core".
path() {
	case "$(origin)" in
		own) printf '%s' "$own" ;;
...
install_version() {
	...
	echo "Installed sing-box $(version_of "$own") for Aether at $own ($(( $(wc -c < "$own") / 1048576 )) MB)"
	[ "$(uci -q get aether.main.enabled)" = 1 ] && /etc/init.d/aether restart >/dev/null 2>&1
	return 0
}
...
sample() {
	...
	mkdir -p "$state_dir"
	current=$(/usr/libexec/aether/traffic.sh 2>/dev/null |
